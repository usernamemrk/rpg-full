export class SpellFX {
    async createPhaserScene(width, height) {
        ...
        return new Promise(resolve => {
            ref.game = new Phaser.Game({
                ...
                transparent: true,
                scene: {
                    create() { resolve({ game: ref.game, scene: this, canvas }); }
                }
            });
        });
    }
    cleanup(game, canvas, delayMs) {
        setTimeout(() => {
            if (game)
                game.destroy(true);
            canvas.remove();
    ...
    async lightning(worldX, worldY, worldTargetX, worldTargetY, camera) {
    ...
    async explosion(worldX, worldY, camera, radius = 64) {
    ...
    async healing(worldX, worldY, camera) {
        const { x, y } = camera.worldToScreen(worldX, worldY);
        const { game, scene, canvas } = await this.createPhaserScene(camera.viewW, camera.viewH);
        const gfx = scene.add.graphics();
        let frame = 0;
...
//# sourceMappingURL=SpellFX.js.map